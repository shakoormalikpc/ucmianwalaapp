...
import { Users, UserCheck, Crown, CreditCard, Heart, Receipt, Wallet, Clock } from "lucide-react";
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      p.forEach((r) => {
        const mi = new Date(r.payment_date).getMonth();
        monthMap[months[mi]].payments += Number(r.amount);
      });
      d.forEach((r) => {
        const mi = new Date((r as any).donation_date || new Date()).getMonth();
        monthMap[months[mi]].donations += Number(r.amount);
      });
      e.forEach((r) => {
        const mi = new Date((r as any).expense_date || new Date()).getMonth();
        monthMap[months[mi]].expenses += Number(r.amount);
      });
